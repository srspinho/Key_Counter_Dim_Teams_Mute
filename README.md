...
O projeto utiliza um Raspberry Pi Pico (RP2040) operando a 120MHz. Abaixo estão as conexões necessárias para o Display OLED e a interface de Teclado/PC.

Pino OLED,Pino Pico (GPIO),Pino Físico,Função
VCC,3V3 Out,Pino 36,Alimentação 3.3V
GND,GND,Pino 38,Terra
SCL,GPIO 5,Pino 7,I2C0 SCL (Clock)
SDA,GPIO 4,Pino 6,I2C0 SDA (Data)

2. Interface USB Host (Para o Teclado Externo)
Utilizamos a biblioteca Pico-PIO-USB para transformar pinos comuns em uma porta USB para o teclado. Recomendamos o uso de um adaptador USB-OTG ou conector USB-A fêmea.

Conexão USB,Pino Pico (GPIO),Pino Físico,Observação
USB D+,GPIO 16,Pino 21,Resistor 22Ω opcional
USB D-,GPIO 17,Pino 22,Resistor 22Ω opcional
USB 5V,VBUS,Pino 40,Alimenta o teclado externo
USB GND,GND,Pino 23,Terra comum

3. Conexão com o PC (USB Device)
A conexão principal do Raspberry Pi Pico (porta Micro-USB/USB-C nativa) deve ser ligada diretamente ao seu Computador.

O Pico será reconhecido como um Teclado HID e um Consumer Control Device (Mídia/Volume).

⚠️ Notas Importantes de Hardware:
Frequência da CPU: O firmware deve ser compilado a 120MHz. O clock de 200MHz ou 133MHz causará instabilidade no USB Host e no I2C.

Alimentação: Certifique-se de que o teclado externo não consome mais do que a porta USB do seu PC pode fornecer (geralmente 500mA), pois ele é alimentado pelo pino VBUS do Pico.

Resistores de Pull-up: A maioria dos módulos OLED SSD1306 já possui resistores de pull-up para o I2C. Se a tela não for detectada, adicione resistores de 4.7kΩ entre SDA/SCL e o 3.3V.
